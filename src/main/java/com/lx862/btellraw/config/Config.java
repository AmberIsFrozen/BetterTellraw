...
public class Config {
    ...
    public static int load() {
        if(!Files.exists(TELLRAWS_DIR.toPath())) {
            generateConfig();
        ...
        int loadedTellraw = 0;
        for (File file : Objects.requireNonNull(TELLRAWS_DIR.listFiles())) {
    ...
    public static void generateConfig() {
        ...
        jsonConfig.addProperty("example2", "<yellow>This is a default tellraw written with <click open_url value 'https://placeholders.pb4.eu/user/quicktext/'><hover show_text 'Hover Text'><u>Placeholder's API QuickText Format</u></hover></click>");
    ...
    public static void saveConfig() {
        ...
        for (Map.Entry<String, TellrawEntry> tellraws : tellraws.entrySet()) {
    ...
    public static int readTellraws(Path tellrawLocation) {
        int loadedTellraw = 0;
        try {
            final JsonObject jsonConfig = JsonParser.parseString(String.join("", Files.readAllLines(tellrawLocation))).getAsJsonObject();
            ...
            for(Map.Entry<String, JsonElement> e : jsonConfig.entrySet()) {
                ...
                tellraws.put(fullID, tellrawObj);
                loadedTellraw++;
            }
            return loadedTellraw;
        } catch (Exception e) {
